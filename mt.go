...
func verify_circuit(proofs []ProofElement) {
	...
	for i := range proofs {
		var numOfLeavesProved = 2 //len(proofs[i].A.LeafIndexes)
		println(i)
		println(numOfLeavesProved)
		// var treeHeight = 2
		// var authPaths = make([][]KeccakDigest, numOfLeavesProved)
		// var prevPath = proofs[i].A.AuthPathsSuffixes[0]
		// authPaths[0] = reverse(prevPath)

		// fmt.Println(authPaths[0])
		// fmt.Println("=================")
		...
		// 	authPaths[j] = reverse(prevPath)
		// 	fmt.Println(authPaths[j])
		// 	fmt.Println("==============")
		// }
		// fmt.Println(proofs[i].A.AuthPathsSuffixes[0])
